    ...
    <>
      ...
      <div className="detail-grid">
        <aside className="detail-rail"><dl>
          ...
          <div><dt>Checked</dt><dd>{connection?.lastCapabilityCheckAt ? new Date(connection.lastCapabilityCheckAt).toLocaleString() : "Never"}</dd></div>
        </dl></aside>
        <div className="detail-content">
          ...
          {connection && <section><h2>Recent synchronization</h2>{runs.length ? runs.map((run) => <div className="sync-summary" key={run.id}><strong>{run.status}</strong><span>{run.triggerType}</span><span>{run.inserted} in / {run.updated} up / {run.failed} failed</span><time>{new Date(run.createdAt).toLocaleString()}</time></div>) : <p className="empty-state">No synchronization runs yet.</p>}</section>}
